...
class MNISTVAE:
    ...
    Decoder: D(z) = neural_net(z) ∈ [0,1]^784  (28×28 MNIST image)
    Forward model: y = D(z) + n, n ~ N(0, σ_n² I)

    The decoder is a trained MLP VAE. Unlike the toy NonlinearDecoder2D,
    this has a realistic nonlinear decoder with 784-dimensional output,
    making it a challenging testbed for latent inverse problem methods.

    Weights are loaded from lip/data/vae_mnist_d{latent_dim}.npz.
    ...
    def decoder(self, z):
        """Neural network decoder D: ℝ^d_latent → [0,1]^784.
    ...
    def encoder(self, x, *, n_iter=10):
        """VAE encoder: returns posterior mean μ(x).
    ...
    def score(self, z, sigma):
        """Score of the noised prior N(0, σ_0² I): ∇_z log p_σ(z)."""
        return -z / (self.sigma_0**2 + sigma**2)
    ...
    def log_prior(self, z):
        """Log prior: z ~ N(0, σ_0² I)."""
        return -0.5 * jnp.sum(z**2, axis=-1) / self.sigma_0**2

    def log_likelihood(self, z, y):
        """Log likelihood: y ~ N(D(z), σ_n² I)."""
        residual = y - self.decoder(z)
        return -0.5 * jnp.sum(residual**2, axis=-1) / self.sigma_n**2
